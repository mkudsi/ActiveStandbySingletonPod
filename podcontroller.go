...
var myPodName string = "InvalidPodName"
var am_I_active_icr_pod bool = false
...
func (c *controller) makeActive(pod *corev1.Pod) bool {
	...
	fmt.Println("### Making active the POD: ", pod.Name, " with IP : ", pod.Status.PodIP)
...
func (c *controller) get_actv_stdby() (actvpod corev1.Pod, stdbypod corev1.Pod,
	...
	podlist, err := c.clientset.CoreV1().Pods("default").List(ctx, metav1.ListOptions{LabelSelector: "app in (hapod)"})
	if err != nil {
		fmt.Println("Error while retrieving pod list during delete: \n", err.Error())
		return
	}
...
/* Post becoming active, need to do reconciliation and populate local data structures with
 * information from etcd.
 */
func (c *controller) icr_post_failover_reconcile() bool {
	ctx := context.Background()

	podlist, err := c.clientset.CoreV1().Pods("default").List(ctx, metav1.ListOptions{LabelSelector: "app in (hapod)"})
	if err != nil {
		fmt.Println("Error while retrieving pod list during reconciliation: \n", err.Error())
		return false
	}

	fmt.Println("Reconciliation: Total pods: ", len(podlist.Items))

	for _, ppod := range podlist.Items {
		if ppod.Labels["role"] == "active" {
			fmt.Println("Reconciliation: Pod: ", ppod.Name, " is active")
			activePodSelected = true
			currActivePodName = ppod.Name
		} else if ppod.Labels["role"] == "standby" {
			fmt.Println("Reconciliation: Pod: ", ppod.Name, " is standby")
		} else {
			fmt.Println("Reconciliation: Pod: ", ppod.Name, " is neither active nor standby. Problem with pod definition file.")
		}
	}

	// TODO : If there are race conditions like multiple failures, we may have gone in a bad state.
	//        If the desired active/standby state of the ICR pods is not proper, then back off for some time
	//        and then do the active/standby selection freshly.

	return true
}

func (c *controller) handle_queued_item() bool {

	// This is a blocking call. It will wait until something is put in the queue.
	item, shutdown := c.queue.Get()
	if shutdown {
		return false
	}

	// TODO : I probably need some synchronization/mutex mechanism here as some events are not handled via the queue.

	defer func() {
		fmt.Println("Returning from handle_queued_item at: ", time.Now())
		c.queue.Forget(item)
	}()

	thispod := item.(*corev1.Pod)

	fmt.Println("\n\n Dequeued : Processing POD: ", thispod.Name, " at : ", time.Now())
	...
	if apierrors.IsNotFound(err) {
		fmt.Println("Handling delete event for POD: ", thispod.Name, " at time: ", time.Now())
		...
		if thispod.Labels["role"] == "active" {
			...
			fmt.Println("Standby POD: ", thispod.Name, " went down. It will be re-created by replicaset. Nothing to do here.")
		}
	} else {
		thispod = tmppod
		...
		if thispod.Labels["role"] == "active" { // If this pod being added is an active pod
			if actvfound == true && actvpod.Name != thispod.Name {
				fmt.Println("A new pod active pod is created and there is already another active pod: ", actvpod.Name)
			}
			// TODO : To somehow resolve this mismatch...
			// This probably can happen when the custom controller itself restarts.
			// Same issue situation can be detected during icr reconcile as well.
			// Write common method to handle such undesired situations.
...
func (c *controller) handleAdd(obj interface{}) {
	...
	if isICRPod(thispod) == true {
		c.handleICRAdd(thispod)
	} else {
...
func (c *controller) handleUpdate(old, cur interface{}) {
	...
	if isICRPod(oldp) == true {
		c.handleICRUpdate(oldp)
		return
	}
	...
	for _, oldstat := range oldp.Status.ContainerStatuses {
		...
		for _, curstat := range curp.Status.ContainerStatuses {
			if curstat.Name == oldstat.Name {
				...
				if oldstat.Ready != curstat.Ready {
					fmt.Println("@@@ Pod: ", oldp.Name, " Container: ", oldstat.Name, " Ready status changed from: ", oldstat.Ready, " to: ", curstat.Ready)
				}
			}
		}
...
func (c *controller) worker() {
	...
	if am_I_active_icr_pod == false {
		//fmt.Println("Ignoring event as I am not yet active ICR: ", myPodName)
		return
	}
	for c.handle_queued_item() {
		// The function handle_queued_item will dequeue an item from queue and process it.
...
func (c *controller) run(ch <-chan struct{}) {
	...
	fmt.Println("Starting Internal Controller. Pod: ", myPodName)
